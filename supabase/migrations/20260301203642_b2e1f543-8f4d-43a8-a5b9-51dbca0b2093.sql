...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      IF col.data_type = 'USER-DEFINED' THEN result := result || col.udt_name;
      ELSIF col.data_type = 'character varying' AND col.character_maximum_length IS NOT NULL THEN result := result || 'varchar(' || col.character_maximum_length || ')';
      ELSE result := result || col.data_type;
      END IF;
      IF col.is_nullable = 'NO' THEN result := result || ' NOT NULL'; END IF;
      IF col.column_default IS NOT NULL THEN result := result || ' DEFAULT ' || col.column_default; END IF;
      result := result || E'\n';
    END LOOP;
    ...
    FOR idx IN
      SELECT indexname FROM pg_indexes WHERE schemaname = 'public' AND tablename = tbl.table_name AND indexname NOT LIKE '%_pkey'
    LOOP
      result := result || 'INDEX: ' || idx.indexname || E'\n';
    END LOOP;

    FOR pol IN
      SELECT polname, polcmd FROM pg_policies WHERE schemaname = 'public' AND tablename = tbl.table_name
    LOOP
      result := result || 'RLS: ' || pol.polname || ' (' || pol.polcmd || ')' || E'\n';
    END LOOP;

    FOR trg IN
      SELECT trigger_name FROM information_schema.triggers
      WHERE event_object_schema = 'public' AND event_object_table = tbl.table_name
    ...
    FOR col IN
      ...
      WHERE table_schema = 'public' AND table_name = tbl.table_name
        AND (column_name LIKE '%type%' OR column_name LIKE '%status%' OR column_name LIKE '%category%'
          OR column_name LIKE '%role%' OR column_name LIKE '%direction%' OR column_name = 'locale' OR column_name LIKE '%_key')
  ...
  FOR tbl IN
    ...
    WHERE routine_schema = 'public' AND routine_type = 'FUNCTION'
      AND routine_name NOT LIKE 'validate_%' AND routine_name NOT LIKE 'update_%'
